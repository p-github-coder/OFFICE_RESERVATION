...
function loadRooms() {
    ...
    available_input.innerHTML = "";

    let bookedRooms =
        JSON.parse(localStorage.getItem("bookedRooms")) || [];

    let availableRooms = rooms.filter(function (room) {
    ...
    if (availableRooms.length === 0) {
        ...
        available_input.appendChild(option);

        available_input.disabled = true;

    } else {

        available_input.disabled = false;

        let defaultOption = document.createElement("option");

        defaultOption.text = "Select Available Room";
        defaultOption.value = "";

        available_input.appendChild(defaultOption);
...
btn_submit.addEventListener("click", function (e) {
    ...
    if (number_input.value == "" || number_input.value == null) {
        ...
        number_input_error.innerHTML = "Phone number required";
    ...
    if (isValid) {
        ...
        bookedRooms.push(available_input.value);
